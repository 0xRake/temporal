...
import (
	...
	"go.temporal.io/server/chasm/lib/workersession"
...
func (h *Handler) RecordWorkerHeartbeat(
...
) (_ *historyservice.RecordWorkerHeartbeatResponse, retErr error) {
	defer log.CapturePanic(h.logger, &retErr)
	...
	if request.GetWorkerInstanceKey() == "" {
		return nil, serviceerror.NewInvalidArgument("WorkerInstanceKey not set on request.")
	}

	if request.GetLeaseExpirationTime() == nil {
		return nil, serviceerror.NewInvalidArgument("LeaseExpirationTime not set on request.")
	}

	// Create or update WorkerSession CHASM entity
	err := h.createOrUpdateWorkerSession(ctx, request)
	if err != nil {
		return nil, h.convertError(err)
	}

	return &historyservice.RecordWorkerHeartbeatResponse{}, nil
}

func (h *Handler) createOrUpdateWorkerSession(
	ctx context.Context,
	request *historyservice.RecordWorkerHeartbeatRequest,
) error {
	entityKey := chasm.EntityKey{
		NamespaceID: request.GetNamespaceId(),
		BusinessID:  request.GetWorkerInstanceKey(),
		// EntityID will be auto-generated by CHASM
	}

	leaseDeadline := request.GetLeaseExpirationTime().AsTime()

	// Try to update existing WorkerSession first
	_, _, err := chasm.UpdateComponent(
		ctx,
		chasm.NewComponentRef[*workersession.WorkerSession](entityKey),
		func(ws *workersession.WorkerSession, ctx chasm.MutableContext, _ any) (any, error) {
			return nil, workersession.RecordHeartbeat(ctx, ws, leaseDeadline)
		},
		nil, // no input needed
	)

	if err != nil {
		// If WorkerSession doesn't exist, create new one
		if serviceerror.IsNotFound(err) {
			_, _, _, err = chasm.NewEntity(
				ctx,
				entityKey,
				func(ctx chasm.MutableContext, _ any) (*workersession.WorkerSession, any, error) {
					ws := workersession.NewWorkerSession()
					err := workersession.RecordHeartbeat(ctx, ws, leaseDeadline)
					return ws, nil, err
				},
				nil, // no input needed
				chasm.WithRequestID(request.GetWorkerInstanceKey()),
			)
		}
	}
